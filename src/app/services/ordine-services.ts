...
export class OrdineServices {
  ...
  getAllByUserId(userId: number) {
    return this.http.get<OrdineDTO[]>(
      this.getBaseUrl() + "getAllByUserId/" + userId,
    );
  }

  getAllByVenditore(userId: number) {
    return this.http.get<OrdineDTO[]>(
      this.getBaseUrl() + "venditore/" + userId,
    );
  }

  getStati() {
    return this.http.get<StatoOrdineDTO[]>(
      this.settings.apiUrl + "StatoOrdine/getAll",
    );
  }
}
